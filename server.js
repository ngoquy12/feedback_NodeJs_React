...
server.get("/api/v1/feedbacks", (req, res) => {
...
server.get("/api/v1/feedbacks/:id", (req, res) => {
  let { id } = req.params;
  // Câu lệnh truy vấn lấy thông tin tất cả bản ghi
...
server.delete("/api/v1/feedbacks/:id", (req, res) => {
  let { id } = req.params;
  // Câu lệnh truy vấn lấy thông tin tất cả bản ghi
...
server.post("/api/v1/feedbacks", validateData, (req, res) => {
  // Lấy dữ liệu từ body
  const { Content, Points } = req.body;
  // Tạo một dữ liệu mới
  const newFeedback = [Content, Points];

  // Viết câu lệnh query string
  const queryString = "insert into feedback(Content, Points) values (?, ?);";
...
server.put("/api/v1/feedbacks/:id", validateData, (req, res) => {
  // Lấy id từ params
  const { id } = req.params;
  ...
  const { Content, Points } = req.body;
  // Tạo một dữ liệu mới
  const newFeedback = [Content, Points, id];

  // Viết câu lệnh query string
  const queryString =
    "update feedback set Content=?, Points=? where FeedbackId=?";
